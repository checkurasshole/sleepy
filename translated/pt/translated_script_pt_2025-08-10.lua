...
local Window = Fluent:CreateWindow({
    Title = "COMBO_WICK DOMINATOR - OTIM...",
    SubTitle = "Desempenho Livre de Atrasos",
    TabWidth = 180,
...
local Tabs = {
    Main = Window:AddTab({ Title = "Principal", Icon = "zap" }),
    Tools = Window:AddTab({ Title = "Ferramentas", Icon = "wrench" }),
    Settings = Window:AddTab({ Title = "Definições", Icon = "cog" })
}
...
local function unlockEverything()
    ...
    Fluent:Notify({
        Title = "FERRAMENTAS DESBLOQUEADAS",
        Content = "Adicionado" .. addedCount .. " tools!",
        Duration = 3
    })
end
...
local function debugPlotInfo()
    ...
    if plot then
        ...
        if platforms then
            Fluent:Notify({
                Title = "PLATFORMS",
                Content = "Encontrado" .. #platforms:GetChildren() .. " platforms",
                Duration = 3
            })
        end
    else
...
Tabs.Main:AddToggle("AutoInsert", {
    Title = "Inserir automaticamente o $",
    Default = false,
    Callback = function(value) 
        autoInsert = value 
        if value then
            Fluent:Notify({Title = "Inserir automaticamente o $", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("InsertSpeed", {
    Title = "Velocidade de inserção",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoCollect", {
    ...
    Callback = function(value) 
        autoCollect = value 
        if value then
            Fluent:Notify({Title = "RECOLHA AUTOMÁTICA", Content = "Enabled!", Duration = 2})
        end
    end
})
...
Tabs.Main:AddToggle("AutoSell", {
    ...
    Callback = function(value) 
        autoSell = value 
        if value then
            Fluent:Notify({Title = "VENDA AUTOMÁTICA", Content = "Enabled!", Duration = 2})
        end
    end
})
...
Tabs.Main:AddButton({
    Title = "Otimização das definições",
    Callback = function()
        insertSpeed = 0.5
        collectSpeed = 0.3
        sellInterval = 1.5
        Fluent:Notify({Title = "OTIMIZADO", Content = "Settings optimized for performance!", Duration = 3})
    end
})

-- Tools Tab (removed manual buttons, kept essential tools)
Tabs.Tools:AddButton({
    Title = "Informações do gráfico de d...",
    Callback = function() debugPlotInfo() end
})

Tabs.Tools:AddButton({
    Title = "Desbloquear todas as ferram...",
    Callback = function() unlockEverything() end
})

Tabs.Tools:AddButton({
    Title = "Teletransportar para o Melh...",
    Callback = function()
        pcall(function()
            ...
            if bestDummy and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                Fluent:Notify({Title = "Teletransporte", Content = "Value: " .. highestAmount, Duration = 3})
            end
        end)
    end
})
...
Tabs.Settings:AddToggle("AutoRejoin", {
    Title = "Reingressar automaticamente",
    Default = true,
...
Fluent:Notify({
    Title = "COMBO_WICK OTIMIZADO",
    Content = "Script sem lag carregado com sucesso!",
    Duration = 5
})
