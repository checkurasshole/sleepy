...
local Window = Fluent:CreateWindow({
    Title = "DOMINADOR COMBO_WICK - OPTI...",
    SubTitle = "Desempeño libre de lag",
    TabWidth = 180,
...
local Tabs = {
    ...
    Tools = Window:AddTab({ Title = "Herramientas", Icon = "wrench" }),
    Settings = Window:AddTab({ Title = "Configuración", Icon = "cog" })
}
...
local function unlockEverything()
    ...
    Fluent:Notify({
        ...
        Content = "Añadido" .. addedCount .. " tools!",
        Duration = 3
    })
end
...
local function debugPlotInfo()
    ...
    if plot then
        ...
        Fluent:Notify({
            Title = "ERROR",
            Content = "¡No se ha encontrado la trama!",
            Duration = 5
        })
    end
end
...
Tabs.Main:AddToggle("AutoInsert", {
    ...
    Callback = function(value) 
        autoInsert = value 
        if value then
            Fluent:Notify({Title = "AUTO INSERT", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("InsertSpeed", {
    Title = "Insertar velocidad",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoCollect", {
    ...
    Callback = function(value) 
        autoCollect = value 
        if value then
            Fluent:Notify({Title = "AUTO COLLECT", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("CollectSpeed", {
    Title = "Recoger velocidad",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoSell", {
    ...
    Callback = function(value) 
        autoSell = value 
        if value then
            Fluent:Notify({Title = "AUTO VENDER", Content = "Enabled!", Duration = 2})
        end
    end
})
...
Tabs.Main:AddButton({
    Title = "Optimizar la configuración",
    Callback = function()
        insertSpeed = 0.5
        collectSpeed = 0.3
        sellInterval = 1.5
        Fluent:Notify({Title = "OPTIMIZADO", Content = "Settings optimized for performance!", Duration = 3})
    end
})

-- Tools Tab (removed manual buttons, kept essential tools)
Tabs.Tools:AddButton({
    Title = "Información del gráfico de ...",
    Callback = function() debugPlotInfo() end
})

Tabs.Tools:AddButton({
    Title = "Desbloquear todas las herra...",
    Callback = function() unlockEverything() end
})

Tabs.Tools:AddButton({
    Title = "Teletransportar al mejor tonto",
    Callback = function()
        pcall(function()
            ...
            if bestDummy and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                Fluent:Notify({Title = "TELEPORTADO", Content = "Value: " .. highestAmount, Duration = 3})
            end
        end)
    end
})
...
Tabs.Settings:AddToggle("AutoRejoin", {
    Title = "Reunirse automáticamente",
    Default = true,
...
Fluent:Notify({
    Title = "COMBO_WICK OPTIMIZADO",
    Content = "¡Script libre de lag cargado con éxito!",
    Duration = 5
})
