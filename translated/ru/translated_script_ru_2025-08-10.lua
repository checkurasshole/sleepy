...
local Window = Fluent:CreateWindow({
    Title = "COMBO_WICK DOMINAtor - ОТВЕ...",
    SubTitle = "Функционирование без лаг",
    TabWidth = 180,
...
local Tabs = {
    Main = Window:AddTab({ Title = "Главное", Icon = "zap" }),
    Tools = Window:AddTab({ Title = "Инструменты", Icon = "wrench" }),
    Settings = Window:AddTab({ Title = "Параметры", Icon = "cog" })
}
...
local function unlockEverything()
    ...
    Fluent:Notify({
        Title = "ТАБЛИЦА ОТКРЫТАЯ",
        Content = "Добавлено" .. addedCount .. " tools!",
        Duration = 3
    })
end
...
local function debugPlotInfo()
    ...
    if plot then
        Fluent:Notify({
            Title = "ПЛОСКИЙ ФУНКЦИОНИРОВАНИЕ",
        ...
        if platforms then
            Fluent:Notify({
                Title = "PLATFORMS",
                Content = "Найдено" .. #platforms:GetChildren() .. " platforms",
                Duration = 3
            })
        end
    else
...
Tabs.Main:AddToggle("AutoInsert", {
    ...
    Callback = function(value) 
        autoInsert = value 
        if value then
            Fluent:Notify({Title = "Ауто Инсерт", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("InsertSpeed", {
    Title = "Вставить скорость",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoCollect", {
    ...
    Callback = function(value) 
        autoCollect = value 
        if value then
            Fluent:Notify({Title = "Ауто-коллект", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("CollectSpeed", {
    Title = "Соберите скорость",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoSell", {
    ...
    Callback = function(value) 
        autoSell = value 
        if value then
            Fluent:Notify({Title = "Продажа AUTO", Content = "Enabled!", Duration = 2})
        end
    end
})
...
Tabs.Main:AddButton({
    Title = "Оптимизировать параметры",
    Callback = function()
        insertSpeed = 0.5
        collectSpeed = 0.3
        sellInterval = 1.5
        Fluent:Notify({Title = "ОПЕРАТИВНАЯ", Content = "Settings optimized for performance!", Duration = 3})
    end
})

-- Tools Tab (removed manual buttons, kept essential tools)
Tabs.Tools:AddButton({
    Title = "Информация об отладочной по...",
    Callback = function() debugPlotInfo() end
})

Tabs.Tools:AddButton({
    Title = "Отключить все инструменты",
    Callback = function() unlockEverything() end
})

Tabs.Tools:AddButton({
    Title = "Телепорт в лучший тупица",
    Callback = function()
        pcall(function()
            ...
            if bestDummy and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                Fluent:Notify({Title = "РУКОВОДЯЩИЕ ПРИНЦИПЫ", Content = "Value: " .. highestAmount, Duration = 3})
            end
        end)
    end
})
...
Fluent:Notify({
    Title = "COMBO_WICK OTIMisised",
    Content = "Сценарий без лаг успешно загружен!",
    Duration = 5
})
