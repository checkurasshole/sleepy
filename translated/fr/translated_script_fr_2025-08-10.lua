...
local Tabs = {
    Main = Window:AddTab({ Title = "Principales", Icon = "zap" }),
    Tools = Window:AddTab({ Title = "Outils", Icon = "wrench" }),
    Settings = Window:AddTab({ Title = "Paramètres", Icon = "cog" })
}
...
local function unlockEverything()
    ...
    Fluent:Notify({
        Title = "OUTILS NON LOUÉS",
        Content = "Ajouté" .. addedCount .. " tools!",
        Duration = 3
    })
end
...
local function debugPlotInfo()
    ...
    if plot then
        ...
        if platforms then
            Fluent:Notify({
                Title = "PLATFORMS",
                Content = "Trouvé" .. #platforms:GetChildren() .. " platforms",
                Duration = 3
            })
        end
    else
...
Tabs.Main:AddToggle("AutoInsert", {
    Title = "Insertion automatique",
    Default = false,
    Callback = function(value) 
        autoInsert = value 
        if value then
            Fluent:Notify({Title = "AUTO INSCRIRE", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("InsertSpeed", {
    Title = "Insérer la vitesse",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoCollect", {
    ...
    Callback = function(value) 
        autoCollect = value 
        if value then
            Fluent:Notify({Title = "AUTO COLLECTE", Content = "Enabled!", Duration = 2})
        end
    end
})

Tabs.Main:AddSlider("CollectSpeed", {
    Title = "Collecter la vitesse",
    Min = 0.1,
    Max = 5,
    Default = 0.5,
    Rounding = 1,
...
Tabs.Main:AddToggle("AutoSell", {
    ...
    Callback = function(value) 
        autoSell = value 
        if value then
            Fluent:Notify({Title = "VENDRE L'AUTO", Content = "Enabled!", Duration = 2})
        end
    end
})
...
Tabs.Main:AddButton({
    ...
    Callback = function()
        ...
        collectSpeed = 0.3
        sellInterval = 1.5
        Fluent:Notify({Title = "OPTIMISÉ", Content = "Settings optimized for performance!", Duration = 3})
    end
})
...
Tabs.Tools:AddButton({
    Title = "Déverrouillage de tous les ...",
    Callback = function() unlockEverything() end
})

Tabs.Tools:AddButton({
    Title = "Téléporter vers Best Dummy",
    Callback = function()
        pcall(function()
            ...
            if bestDummy and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                Fluent:Notify({Title = "TÉLÉPORTÉS", Content = "Value: " .. highestAmount, Duration = 3})
            end
        end)
    end
})
...
Tabs.Settings:AddToggle("AutoRejoin", {
    Title = "Rejoins-nous automatiquement",
    Default = true,
...
Fluent:Notify({
    Title = "COMBO_WICK OPTIMISÉ",
    Content = "Le script sans lag est chargé avec succès !",
    Duration = 5
})
